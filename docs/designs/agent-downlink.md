...
- A replacement machine set up under a previous machine's name and password adds to that
  machine's area and cannot damage it, because nothing deletes in the bucket.
...
| Mutation | gremlins mutates the whole module on every pull request; `tools/gremlinsgate` fails the run on any mutant not killed, unless `.gremlins-equivalents.json` records a reviewed proof that no test can distinguish it. Scope is in `.gremlins.yaml`. See ADR-005. |
